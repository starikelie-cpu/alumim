...
app.put('/api/synagogues/:id', requireAdmin, async (req, res) => {
    try {
        ...
        if (normalizeRole(user.role) === 'synagogue_admin') {
            ...
            // Allow name, website, and logo fields to be changed by synagogue_admin
...
app.post('/api/members/self-register', async (req, res) => {
    try {
        ...
        const synMap = prefs.synagogueSelfReg || {};
        const strId = String(synagogueId);
        const numId = Number(synagogueId);
        const synStatus = synMap[strId] !== undefined ? synMap[strId] : (!isNaN(numId) && synMap[numId] !== undefined ? synMap[numId] : true);
