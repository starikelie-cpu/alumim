const { app, BrowserWindow, utilityProcess, protocol, dialog } = require('electron');
    ...
    return new Promise((resolve, reject) => {
        ...
        serverProcess.on('exit', (code) => {
            ...
            if (code !== 0 && code !== null) {
                ...
                waitForHttpServerReady('http://localhost:3000/api/db-status', 5000, 500)
                    .then(() => {
...
app.whenReady().then(async () => {
    try {
        ...
        await waitForHttpServerReady('http://localhost:3000/api/db-status', 30000, 500);
